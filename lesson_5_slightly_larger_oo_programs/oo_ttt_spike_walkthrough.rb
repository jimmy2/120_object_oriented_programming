...
class Board
  ...
  def unmarked_keys
    @squares.keys.select { | key, _ | @squares[key].unmarked? }
  end
  ...
  def detect_winner
    WINNING_LINES.each do | line |
      if count_human_marker(@squares.values_at(*line)) == 3 # @squares.select { |k,_| line.include?(k) }.values
        return TTTGame::HUMAN_MARKER
      elsif count_computer_marker(@squares.values_at(*line)) == 3  # @squares.select { |k,_| line.include?(k) }.values
...
class TTTGame
  ...
  def display_board(clear=true)
    ...
    puts "You're an #{human.marker}.  The computer is an #{computer.marker}."
    puts ""
    puts "     |     | "
  ...
  def human_moves
    puts "Please select a number (#{board.unmarked_keys.join(", ")}):"
    square = nil
    loop do
